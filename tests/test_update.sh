...
################################
# get_rdata, dig_query_short, and the other DNS/lease-store primitives now
# live in tests/lib/*.sh (sourced above); make_rr/build_key_nonkey_rr below
# are specific to this suite's RFC 9664 RR-type matrix (KEY/TXT/A/AAAA/
# WALLET/CLA/IPN) and stay here rather than in a shared lib.
make_rr() {
    local rr_type="$1"
    local ttl="$2"
    local wrong="${3:-0}"
